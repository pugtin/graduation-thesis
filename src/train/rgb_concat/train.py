...
def load_data():
    ...
    for subject in subjects:
        h5path = processed_path / str(subject) / "rgb_concat.h5"
        h5f = h5py.File(h5path, "r")
        shape = h5f["shape"][:]
        encoder = np.array(h5f[f"rgb_concat_{encoding}"][:]).reshape(shape)
        h5f.close()
        # encoder_path = processed_path / str(subject) / "rgb_concat" / f"{encoding}.npy"
        #
        # encoder = np.load(encoder_path).astype("float32")
        X_dict[subject] = encoder
