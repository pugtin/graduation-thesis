...
def load_data():
    ...
    for subject in subjects:
        ...
        shape = h5f["shape"][:]
        x = np.array(h5f[f"{encoding}_x"][:]).reshape(shape)
        y = np.array(h5f[f"{encoding}_y"][:]).reshape(shape)
        z = np.array(h5f[f"{encoding}_z"][:]).reshape(shape)
        h5f.close()
        # x_path = processed_path / str(subject) / encoding / "x.npy"
        # y_path = processed_path / str(subject) / encoding / "y.npy"
        # z_path = processed_path / str(subject) / encoding / "z.npy"
        #
        # x = np.load(x_path).astype("float32")
        # y = np.load(y_path).astype("float32")
        # z = np.load(z_path).astype("float32")
...
#         callbacks=callbacks,
#         validation_data=([X_val[:,0], X_val[:,1], X_val[:,2]], y_val_hot),
#     )
#
#     pred_test_arr = model.predict([X_test["x"], X_test["y"], X_test["z"]])
...
def train_loso():
    ...
    #
    #     X_train = np.asarray([
    #         np.vstack([X_dict[subject]["x"] for subject in train_subjects]),
    #         np.vstack([X_dict[subject]["y"] for subject in train_subjects]),
    #         np.vstack([X_dict[subject]["z"] for subject in train_subjects])
    #     ])
    #
    ...
    log_dir = Path("/Users/wkong0407/Documents/yairi-lab/parker-preprocessing-comparison/results_deprecated/multihead/gasf/20230713-182127")
